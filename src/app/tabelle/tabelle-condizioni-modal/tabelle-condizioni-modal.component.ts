import { Component, OnInit, ViewChild, Input , LOCALE_ID} from '@angular/core';
...
@Component({
  selector: 'app-tabelle-condizioni-modal',
  providers: [TableUtilService, DatiMockTabelleService,{
    provide: LOCALE_ID,
    useValue: 'it-IT' 
  }],
  templateUrl: './tabelle-condizioni-modal.component.html',
  styleUrls: ['./tabelle-condizioni-modal.component.less']
})
export class TabelleCondizioniModalComponent implements OnInit {
  ...
  exampleFormat = (num) =>{
    return (num && (!isNaN(num)))?new Intl.NumberFormat('it-IT', { style: 'currency', currency: 'EUR' }).format(num):num;
  }
  ...
  ngOnInit() {
    ...
    if (this.tipoTab) {
      switch (this.tipoTab.toUpperCase()) {
        case 'GRID': {
          this.tabGrid = true;
          break;
        }
    ...
    this.datiMockTabelleService.getTestataTabella("TabellaCond")
      .then(value => {
        self.columnDescription = value;
        self.fieldsTable = Object.keys(self.columnDescription)
      });

    /* Lettura dati tabella */
    this.datiMockTabelleService.getAllClients("TabellaCond")
      .then(value => {
        self.mockData = [...value];
        ...
        let currentPageStatus = {
          pageNumber: 1,
          pageSize: 5,
          totalRecords: this.datiTabBE.length
        }
