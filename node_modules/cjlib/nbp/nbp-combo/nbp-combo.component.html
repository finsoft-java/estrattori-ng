<div class="dropdown-container"><input type="hidden" [(ngModel)]="value"><div class="dropdown-inner-container" (click)="_onInputClick($event)" (keypress)="_searchByKey($event)" (keydown.arrowdown)="_moveNext($event)" (keydown.arrowup)="_movePrev($event)" (keydown.enter)="_manageDropdownState($event)" (keydown.esc)="_manageDropdownState($event)" (keydown.spacebar)="_onSelectByKeyboard($event)" (keydown.space)="_onSelectByKeyboard($event)" (keydown.tab)="_manageDropdownState($event)" (keydown.shift.tab)="_manageDropdownState($event)" (keyup.shift.tab)="_manageDropdownState($event)" (keyup)="_checkEmpty($event)" (focus)="_focusActive = true;" [ngClass]="{'collapse-disabled' : disabled}" (focusout)="_focusOutFn($event)"><input #visibleTextBox type="text" [id]="id" [name]="name" [placeholder]="placeholder" class="dropdown-input-box form-control" [ngClass]="{
